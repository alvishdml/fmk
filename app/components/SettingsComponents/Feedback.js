...
export default class Feedback extends Component {
  ...
  renderHeader(){
    ...
    <Text style={{
      ...
      marginBottom: 0,
      marginTop: 0 }}>
      {I18n.t('app.components.SettingsComponents.Feedback.sendUsFeedBack')}
      </Text>
      </View>
      </View>
    ...
    sendFeedback(){
      ...
      Meteor.call('sendFeedback', Meteor.user()._id, this.state.feedback,(err, result) => {
        if (err) {
          Alert.showAlert('', errorMessage);
          trackEvent('Send_Feedback', 'Error');
        } else {
          Alert.showAlert('', I18n.t('app.components.SettingsComponents.Feedback.appreciateFeedback'));
          trackEvent('Send_Feedback', 'Success');
        }
      }
    )
    Actions.mainPage();
    ...
    return (
      ...
      style={{
        ...
        <View style={{
          ...
          {I18n.t('app.components.SettingsComponents.Feedback.anyFeedback')}
          </Text>
          <Text style={{margin: 20, fontFamily: 'Montserrat-Light', fontSize:17}}>
          {I18n.t('app.components.SettingsComponents.Feedback.letUsKnow')}
          </Text>
          <TextInput id="inputAbout"
          placeholder={I18n.t('app.components.SettingsComponents.Feedback.aboutPlaceHolder')} multiLine={true} rows={1} rowsMax={10}
          style={{ margin: 20,
            fontFamily:'Montserrat-Light', bottom:40,
             }}
             underlineColorAndroid= {underlineColor}
            onChangeText={(text) => this.setState({feedback: text})}
            textareaStyle={{ fontSize: 14, color: underlineColor }} />
          {this.state.error &&
            <Text style={{margin:23, marginTop: -60, fontFamily: 'Montserrat-Light', fontSize:10, color: '#e74c3c'}}>
            {I18n.t('app.components.SettingsComponents.Feedback.writeMessage')}
            </Text>
          }
            </View>
            </View>
            ...
            <TouchableOpacity onPress={() => {
              ...
              {I18n.t('app.components.SettingsComponents.Feedback.cancel')}
              </Text>
              ...
              <Text style={{ textAlign:'center', color: '#FFFFFF',
              fontSize: 12, fontFamily: 'Montserrat-Light',}}>
              {I18n.t('app.components.SettingsComponents.Feedback.send')}
              </Text>
              </TouchableOpacity>
              </View>
