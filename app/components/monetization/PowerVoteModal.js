...
const purchaseIdDisc = [
  'com.playfmk.powervotes50disc',
  'com.playfmk.powervotes30disc',
  'com.playfmk.powervotes10disc',
];
const purchaseId = [
  'com.playfmk.powervotes50',
  'com.playfmk.powervotes30',
  'com.playfmk.powervotes10',
];

export default class PowerVoteModal extends Component {
  constructor(props) {
    super(props);
    this.state = {
      reason: '',
    ...
    Meteor.call('showDiscount', Meteor.user()._id, async (err, val) => {
      if (!err && val) {
        ...
        try {
          ...
          this.setState({ prices, normalPrices, discount: true });
        } catch (err) {
        } finally {
          await InAppBilling.close();
        }
        ...
        <View style={styles.mainView}>
          ...
          >
            <Image
              ...
              source={{
                uri:
                  'https://fmk-images.ams3.digitaloceanspaces.com/defaults/three-stars.png',
              }}
              style={{
                height: WIDTH * 0.2,
                width: WIDTH * 0.6,
            ...
            >
              {I18n.t(
                'app.components.monetization.MonetizationSlideShowTab.powervoteTitle'
              )}
            </Text>
          </LinearGradient>
          <Text
          ...
          >
            {I18n.t(
              'app.components.monetization.MonetizationSlideShowTab.powervoteDesc'
            )}
          </Text>
          <View
            style={{
          ...
          <TouchableOpacity
            ...
            onPress={async function() {
              ...
              trackEvent('Power_Vote_Screen', 'Click_buy', {
                label: selectedId,
                value: price,
              });

              var purchasedCollection = this.state.purchased;
              this.pay(selectedId, 'powerVoteAll', qty, price);
            }.bind(this)}
          >
            <Text
              style={{
  ...
  componentDidMount() {
    if (this.props.container.state.showDiscount) {
      trackEvent('Monetization', 'BoxClicked2', {
        label: 'com.playfmk.powervotes50disc',
        value: parseInt(this.props.qty),
      });
    } else {
      trackEvent('Monetization', 'BoxClicked2', {
        label: 'com.playfmk.powervotes50',
        value: parseInt(this.props.qty),
      });
    }
  }
